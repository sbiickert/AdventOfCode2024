...
Begin
	...
	ext.Print();
	ext.ExpandToFit(Coord2D.Create(-1,-1));
	ext.Print;
	ext.ExpandToFit(Coord2D.Create(3,12));
	ext.Print;
	...
	WriteLn('Position testing');
	
	p1 := Position2D.Create(Coord2D.Origin, NORTH);
	p1.Print;
	p1.Turn(Rotation2D.CW);
	p1.Print;
	p1.MoveForward(5);
	p1.Print;
	p1.Turn(Rotation2D.CCW);
	p1.Print;
	...
	Begin
		grid.SetString('N', neighbours[i]);
		neighbours[i].Print;
	End;
	grid.SetInteger(5, c1.Add(Coord2D.Offset(EAST)));
	grid.Print;
